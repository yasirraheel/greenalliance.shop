@php
    ...
    $labelAll = trans('plugins/real-estate::filters.all_flats');
    $flats = [
        '0-20'      => '&lt; 30',
        '20-50'     => '20 - 50',
        '50-100'    => '50 - 100',
...
<div class="form-group min-max-input" data-calc="{{ json_encode($calc, true) }}" data-all="{{ $labelAll }}">
    <div class="row">
        <div class="col-5 pr-1">
            <label for="min_flat" class="control-label">{{ trans('plugins/real-estate::filters.flat_from') }}</label>
            <input type="number" name="min_flat" class="form-control min-input" id="min_flat"
                value="{{ request()->input('min_flat')}}" placeholder="{{ trans('plugins/real-estate::filters.from') }}" min="0" step="1">
            <span class="position-absolute min-label d-none"></span>
        </div>
        <div class="col-5 px-1">
            <label for="max_flat" class="control-label">{{ trans('plugins/real-estate::filters.flat_to') }}</label>
            <input type="number" name="max_flat" class="form-control max-input" id="max_flat"
                value="{{ request()->input('max_flat') }}" placeholder="{{ trans('plugins/real-estate::filters.to') }}" min="0" step="1">
